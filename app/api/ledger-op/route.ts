...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Fire-and-forget indexer refresh + cost record. The user's owner
    // X/Y travel in headers (same shape the auth verifier uses) so we
    // can attribute the gas cost without changing request bodies.
