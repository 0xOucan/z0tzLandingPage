...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { chainId, sweep } = body as { chainId: number; sweep: SweepReq };
    if (!chainId || !sweep) return NextResponse.json({ error: "missing chainId or sweep" }, { status: 400, headers: corsHeaders });
    ...
    const { txHash } = await submitSweep(chainId, sweep);
