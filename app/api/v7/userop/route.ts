...
export async function POST(req: NextRequest) {
  ...
  try {
    const { chainId, op } = await req.json();
    if (!chainId || !op) {
    ...
    if (!entryPoint) {
      return NextResponse.json({ error: "dest deployment missing entryPoint" }, { status: 500, headers: v7CorsHeaders });
    }
    ...
    return NextResponse.json({ error: e?.message ?? "userop failed" }, { status: 500, headers: v7CorsHeaders });
  }
}
